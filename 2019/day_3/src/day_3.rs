...
pub fn find_min_intersection(vec_1: Vec<Point>, vec_2: Vec<Point>) -> i64 {
  find_intersections(&vec_1, &vec_2).iter().map(
    |x| x.manhatten_distance(Point::new_point(0, 0))
  ).min().unwrap()
}
...
pub fn find_intersection(
  tail_1: Point, head_1: Point, tail_2: Point, head_2: Point) -> Option<Point> {

  let u_a = (
    (head_2.x - tail_2.x) * (tail_1.y - tail_2.y) - (head_2.y - tail_2.y) * (tail_1.x - tail_2.x)
  ) as f64 / (
    (head_2.y - tail_2.y) * (head_1.x - tail_1.x) - (head_2.x - tail_2.x) * (head_1.y - tail_1.y)
  ) as f64;
  
  let u_b = (
    (head_1.x - tail_1.x) * (tail_1.y - tail_2.y) - (head_1.y - tail_1.y) * (tail_1.x - tail_2.x)
  ) as f64 / (
    (head_2.y - tail_2.y) * (head_1.x - tail_1.x) - (head_2.x - tail_2.x) * (head_1.y - tail_1.y)
  ) as f64;
...
pub fn get_shortest_travel(wire_1: Vec<Point>,  wire_2: Vec<Point>, points: Vec<Point>) -> i64 {
  points.iter().map(
    |point| get_distance_to_point(&wire_1, *point) + get_distance_to_point(&wire_2, *point)
  ).min().unwrap()
}
...
pub fn point_intersects_line(tail: Point, head: Point, point: Point) -> Option<i64> {
  ...
  println!("line {:?} -> {:?} vs point {:?}, leads to {}, {}", tail, head, point, t_x, t_y);
